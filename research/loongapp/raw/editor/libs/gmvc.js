...
(function (gmvc) {
    ...
    class GHistory extends gmvc.GModule {
        ...
        init() {
            super.init();
            // 在这里初始化state，因为state是由上成的store注入的，这里的state已经是store的引用了，里面直接用就行。
            // 如果在构造函数里自己创建state，就上层的store在注入就给覆盖了，导致上层的store拿不到数据。
            if (!this.state) {
                this.state = {
                    enable: false,
                    branches: [{
                ...
                if (this.state.enable === undefined) {
                ...
                if (this.state.branches[0].name === undefined) {
                    this.state.branches[0].name = 'root';
                }
                if (this.state.curBranch === undefined) {
                    this.state.curBranch = 'root';
                }
                if (this.state.curBranch !== this.state.branches[0].name) {
                    this.state.curBranch = this.state.branches[0].name;
                }
            }
        ...
        applyState(state, subState) {
            for (const key in subState) {
                ...
                if (!error) {
                    if (subState[key] === undefined) {
                        delete targetState[targetKey];
                    }
                    else {
        ...
        stopMerge(name) {
            if (this.mergeNumber > 0) {
                ...
                if (this.merging) {
                    console.warn('there is a bug, never run this', name, this.mergeArray);
                    this.merging = false;
                    this.mergingMutations = [];
                }
            }
        ...
        undo() {
            ...
            if (this.curBranch.history.length) {
                ...
                if (undoState.mutation && undoState.mutation.length) {
                    for (let i = undoState.mutation.length - 1; i >= 0; i--) {
        ...
        get mutations() {
            ...
            return {
                ...
                [GHistoryMutation.MERGE_START](state, payload) {
                    that.startMerge((payload === null || payload === void 0 ? void 0 : payload.name) || '');
                },
                [GHistoryMutation.MERGE_END](state, payload) {
                    that.stopMerge((payload === null || payload === void 0 ? void 0 : payload.name) || '');
                },
...
(function (gmvc) {
    class ShortcutController extends gmvc.Controller {
        ...
        removeRegister(type, callback, focus = null) {
            ...
            for (i = 0, len = this._entries.length; i < len; i++) {
                if (this._entries[i].type === type &&
                    ...
                    this.focusEqual(this._entries[i].focus, focus)) {
        ...
        focusEqual(focus1, focus2) {
            if (Array.isArray(focus1) && Array.isArray(focus2)) {
                if (focus1.length === focus2.length) {
                    for (let i = 0, len = focus1.length; i < len; i++) {
                        if (focus1[i] !== focus2[i]) {
                            return false;
                        }
                    }
                    return true;
